    ...
    def apply_to(self, candidate):
        if candidate in self.choices:
            if len(self.current_picks) < self.total_places:
                ...
                self.current_picks = sorted(
                    self.current_picks, key=lambda r: self.choices.index(r))
...
class MatchController():
    ...
    def __init__(self, program_data, candidate_data, places_data=None):
        ...
        program_data_str = StringIO(program_data.decode(
            'utf-8').lower().replace(' ', '').replace('\n', '').replace('\t', ''))
        candidate_data_str = StringIO(candidate_data.decode(
            'utf-8').lower().replace(' ', '').replace('\n', '').replace('\t', ''))
        ...
        self.candidate_data = self.candidate_data.drop(
            self.candidate_data.index[0])
        self.candidate_data = self.candidate_data.drop(
            self.candidate_data.index[0])
        if places_data:
            places_data_str = StringIO(places_data.decode('utf-8'))
            # self.places_data = csv.reader(places_data_str, delimiter=',')
            self.places_data = pd.read_csv(
                places_data_str, names=[1, 2], index_col=0)
    ...
    def organize_candidates(self, places_data):
        try:
            for c in self.program_data.columns:
                ...
                if places_data:
                    self.programs[c] = Program(
                        c, self.places_data.loc[c].at[2])
                else:
